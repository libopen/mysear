...
class smartWizard extends CWidget {
  public  $selected=0,  // Selected Step, 0 = first step   
    ...
    $contentPane='<div id="step-{stepNumber}"><h2 class="StepTitle">{steplabel} {stepNumber}: {StepTitle}</h2>{content}</div>';
            $tabs=$mainContent="";
            $counter=1;
            ...
            <link href="<?php echo $this->styles;?>" rel="stylesheet" type="text/css">
    ...
    public function publishAssets() {
        ...
        if (is_dir($assets)) {
            ...
            //$this->styles=$assets. '/styles/smart_wizard.css';
			    ...
			    //Yii::app() ->clientScript -> registerScriptFile($baseUrl . "/js/jquery.smartWizard-2.0.min.js", CClientScript::POS_HEAD); 
			 ...
			 $(document).ready(function(){
				$("'.$this->selector.'").smartWizard({ 
                      ...
                      beforeRequest:'. strNull($this->beforeRequest) .',
