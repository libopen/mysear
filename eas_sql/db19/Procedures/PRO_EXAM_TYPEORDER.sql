...
begin
  ...
  EXECUTE IMMEDIATE 'truncate table EAS_ExmM_TempTypeOrder'; --清空表数据
  ...
  v_sql_select := v_sql1_create || 'select EEOrder.examplancode,' || --考试计划代码
                  ...
                  'PrintshopNameTable.Printshopname Printshopname,' || --印刷点代码
                  'CountAll.t1 T1,' || --T1
                  'CountAll.cou TCount ' || --单行合计
                  ' from (select CountLocal.*, CountLocal.t1 cou ' || --单行合计
