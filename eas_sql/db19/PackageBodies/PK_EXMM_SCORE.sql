...
BEGIN
        ...
        begin
            ...
            if InExamCategoryCode is null then
                strSql:='select signUp.SN,signUp.ExamCategoryCode,signUp.CourseID,signUp.ExamPaperCode,sss.SignUpNum,signUp.ExamUnit,signUp.AssessMode,signUp.CollegeCode,signUp.LearningCenterCode,signUp.ClassCode,signUp.StudentCode 
                    ...
                    and not exists(
                        select 1 from EAS_ExmM_ComposeScore paperScore where paperScore.examPlanCode = signUp.examPlanCode and paperScore.examCategoryCode = signUp.examCategoryCode
                ...
                loop
                    ...
                    if x_ScoreStandPlan is not null then
                        ...
                        --形考
                        --获取形考的最高成绩
                        select count(1) into x_Count from EAS_ExmM_XKScore where studentCode=x_StudentCode and courseID = x_CourseID and rownum=1 order by Score desc ;
                        if x_Count != 0 then
                        ...
                        (SN,ExamPlanCode,ExamCategoryCode,ExamUnit,CourseID,ExamPaperCode,SegmentCode,CollegeCode,LearningCenterCode,ClassCode,AssessMode,IsComplex,Sign_SN,StudentCode,XKP_SN,NumSignUp,PublishDate,XKS_SN,Paper_SN)
                        values
                        (seq_ExmM_StudentScore.nextVal,InExamPlanCode,x_ExamCategoryCode,x_ExamUnit,x_CourseID,x_ExamPaperCode,InSegmentCode,x_CollegeCode,x_LearningCenterCode,x_ClassCode,x_AssessMode,0,x_SignSN,x_StudentCode,x_ScoreStandPlan,x_NumSignUp,null,x_XKScoreSN,x_PaperScoreSN);
   ...
   PROCEDURE UpdateComposeScore 
    ...
    x_CourseId varchar(20);
    x_StudentCode varchar(20);
    x_MaxScore number(7,2);
    x_MaxScoreCode varchar(20);
    x_ExamUnitType varchar(20);
    x_IsPass int;--是否修完、通过
    BEGIN
       OutCount := 0;
       ...
       LOOP
        ...
        x_CourseId:= XMLDOM.GETNODEVALUE(XMLDOM.GETFIRSTCHILD(XMLDOM.ITEM(chilNodes,9)));
        x_StudentCode:= XMLDOM.GETNODEVALUE(XMLDOM.GETFIRSTCHILD(XMLDOM.ITEM(chilNodes,10)));
        x_ExamUnitType:= XMLDOM.GETNODEVALUE(XMLDOM.GETFIRSTCHILD(XMLDOM.ITEM(chilNodes,11)));
        ...
        ComposeScore= x_ComposeScore,ComposeScoreCode =x_ComposeScoreCode,IsComplex = 1,ComposeDate=sysdate,IsPublish =1,PublishDate = sysdate 
        where Sn = x_SN;
        --更新学生学习状态表
        --获取最高成绩
         select ComposeScore,ComposeScoreCode into x_ComposeScore, x_ComposeScoreCode from (select * from EAS_ExmM_ComposeScore where studentCode=x_studentCode  and composeScore is not null order by composeScore desc) allData where rownum =1;
         if x_ComposeScore is not null then
            if x_ComposeScore >=60 then
               x_IsPass := 4;
            else
               x_IsPass := 3;
            end if;
            update EAS_Elc_StudentStudyStatus set Score = x_ComposeScore,ScoreCode = x_ComposeScoreCode,ScoreType = x_ExamUnitType,StudyStatus = x_IsPass where studentCode = x_StudentCode and courseID = x_CourseID; 
         end if;
        COMMIT;
        OutCount :=OutCount+1;
