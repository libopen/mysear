   ...
   for lists in os.listdir(rootpath):
      ...
      if os.path.isdir(path):
         ...
         if os.path.basename(path)[0]=='S' and os.stat(path).st_size!=0:
            ...
            pathlist.append((path,base[-6:]))            
   return pathlist
   
def readall(rootpath=ROOTPATH,myfind="*"):
   pathlist = getallfile(rootpath)
   if myfind!="*":
      for path,sname in pathlist:
         if sname==myfind:
            pathlist=[(path,sname)]
            break
   
   cursn,dbcurrent=createdb1(pathlist[0][0])
   for i in range(1,len(pathlist)):
      result = dbcurrent
      cursn,dbcurrent=createdb1(pathlist[i][0])
      if dbcurrent is not None:
         dbcurrent=result.append(dbcurrent)
