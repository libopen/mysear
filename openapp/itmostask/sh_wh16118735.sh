...
FILENAME="$1"
a=($(cat $FILENAME))
cp ../module/Tmp_cps_student.txt "$FILENAME".ctl
#echo "${a[@]}"
...
awk -F, -v slist="${a[*]}" -v fname="$FILENAME"  'BEGIN {tlen=split(slist,A," ")} {for(i in A) 
                                                             if (A[i]==$3)
                                                                 print $0>>fname".ctl"  }' <~/data/student/cps_student.dat
#while read -r line
#do
#   name="$line"
